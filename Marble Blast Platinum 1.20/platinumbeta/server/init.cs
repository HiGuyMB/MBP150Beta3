...
function initDedicated()
{
   ...
   //HiGuy: Let people control this server!
...
function fixDedicatedShit() {
   //HiGuy: Things we need
   exec($usermods @ "/leaderboards/main.cs");
   exec($usermods @ "/client/scripts/tcp.cs");
   exec($usermods @ "/client/scripts/miscfunctions.cs");

   //HiGuy: Fix master server
   exec($usermods @ "/leaderboards/multiplayer/defaults.cs");

   initMultiplayerServer();

	//HiGuy: Validate ignition so we can play
	if ($platform $= "x86UNIX")
