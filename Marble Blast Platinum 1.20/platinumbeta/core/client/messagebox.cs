...
function MessageBoxOK( %title, %message, %callback, %errorImage )
{
   //HiGuy: This dialog should be able to be used >_>
   // I added %errorImage so as not to break compatibility
   MBOKErrorImage.setVisible(!%errorImage);
...
function MessageBoxYesNo( %title, %message, %yesCallback, %noCallback )
{
   ...
   MBSetText(MBYesNoText, MBYesNoFrame, "<spush><font:Comic Sans MS Bold:24><just:center>" @ %title @ "<spop>\n" NL %message);
