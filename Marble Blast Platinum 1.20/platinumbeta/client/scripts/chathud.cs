...
function playMessageSound(%message, %voice, %pitch)
{
   ...
   if (strstr(%wavFile, ".ogg") != (strlen(%wavFile) - 4)) {
      %wavFile = %wavFile @ ".ogg";
   }
...
function addChatLine(%message)
{
   if (getWordCount(%message)) {
      ...
      ChatTextBackground.setText("<color:776622>" @ %text);
      ChatTextForeground.setText("<color:ffee99>" @ %text);
...
function addHelpLine(%message, %playBeep)
{
   if (getWordCount(%message)) {
      %text = "<just:center><font:DomCasualD:32>" @ %message;
      HelpTextBackground.setText("<color:777777>" @ %text);
